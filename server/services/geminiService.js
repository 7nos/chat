...
const generateContentWithHistory = async (chatHistory, systemPromptText = null, relevantDocs = []) => {
    try {
        ...
        console.log(`Sending message to Gemini. History length: ${historyForStartChat.length}. System Prompt Used: ${!!modelOptions.systemInstruction}`);

        const result = await retry(async () => {
            return await chat.sendMessage(lastUserMessageText);
        }, {
            retries: 3,
            factor: 2,
            minTimeout: 1000,
