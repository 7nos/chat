...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        ...
        if (!req.user || !req.user._id) { // Check for _id instead of username initially
            // This should ideally not happen if tempAuth works correctly
            console.error("Multer Destination Error: User _id context missing after auth middleware.");
            return cb(new Error("Authentication error: User context not found."));
        }
        // Use user._id for directory naming
        const userIdForDir = req.user._id.toString();
        ...
        // Use userIdForDir in the path
        const destinationPath = path.join(UPLOAD_DIR, userIdForDir, fileTypeSubfolder);
...
router.post('/', tempAuth, (req, res) => {
    ...
    uploader(req, res, async function (err) {
        ...
        // File has been uploaded by multer to req.file.path
        const savedFilePath = req.file.path; // This is the full path where Multer saved the file

        console.log(`[UPLOAD SUCCESS] File saved to: ${savedFilePath} for user: ${userId}`);
        console.log(`[UPLOAD SUCCESS] Original filename: ${originalName}`);

        // --- Trigger RAG Processing ---
        // Pass the absolute path to the RAG service
        const ragProcessingResult = await triggerPythonRagProcessing(userId, savedFilePath, originalName);
